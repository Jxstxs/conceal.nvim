...
local _read_cached_states = function()
    local cache_file = vim.fn.stdpath("cache") .. "/conceal.state"
    local cached_state = {}
    local current_line = nil
    local find_start = nil
    local pre_lang = nil
    local current_lang = nil
    local file = io.open(cache_file, "r")
    if file ~= nil then
        for line in io.lines(cache_file) do
            find_start, _ = string.find(line, "=")
            if find_start == 1 then
                current_lang = split_by(line, "=")[1]
                if current_lang ~= pre_lang then
                    cached_state[current_lang] = {}
                    pre_lang = current_lang
                end
            else
                current_line = split_by(line, ":")
                cached_state[current_lang][current_line[1]] = { _str_to_bool[current_line[2]](), current_line[3] }
            end
        end
        file:close()
    end
    ...
    if file ~= nil then
        for language, keywords in pairs(config) do
            ...
            for keyword, options in pairs(keywords.keywords) do
                file:write(keyword .. ":" .. tostring(options.enabled) .. ":" .. options.conceal .. "\n")
            end
            file:write("=" .. language .. "\n")
        end
        file:close()
